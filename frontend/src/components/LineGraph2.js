...
const LineGraph2 = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:8000/customer_overtime_positive_sentiment/');
        const responseData = await response.json();
        const [labels, data]=[Object.keys(responseData.overtime_sentiment_data), Object.values(responseData.overtime_sentiment_data)];
