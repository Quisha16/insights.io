    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost:8000/sentiment_prediction/"
        );
        const responseData = await response.json();
        const [labels, data] = [
          Object.keys(responseData.prediction_data),
          Object.values(responseData.prediction_data),
        ];
    ...
    <div className="card">
      <div className="info-container">
        <div className="info-box">
          <span className="info-label total">Total Reviews</span>
          <span className="info-value info-value-total">{totalReviews}</span>
        </div>
        <div className="info-box">
